...
# %%
def optimize_combine_1(uniques: sparse.csr_matrix, times_uniques: list, dims: tuple, Params: dict, filename_GT: str, filename_GT2, filename_GT3):  
   ...
   # fname = filename_GT2.replace('_sparse.mat','.mat')
    #GTMasks_2 = np.array(h5py.File(filename_GT2, 'r')['FinalMasks'])
    GTMasks_2 = data_GT['GTMasks_2'].transpose()
    #GTMasks_2 = np.transpose(GTMasks_2,(2,0,1))
   ...
   # rot_m_1 = np.rot90(GTMasks_2, axes = (1,2))
    
  #  rot_m_2 = np.rot90(rot_m_1, axes = (1,2))
    
 #   rot_m_3 = np.rot90(rot_m_2, axes = (1,2))
    
  #  GTMasks_2 = np.concatenate((GTMasks_2,rot_m_1, rot_m_2, rot_m_3),0)
    num = np.shape(GTMasks_2)[0]
    
  #  GTMasks_2 = GTMasks_2.astype(np.uint8)
    
    # f2 = h5py.File(filename_GT3, "w")
    # f2.create_dataset("timesfinal", data = times_piecedneurons)
    # f2.close()
   # print(times_piecedneurons)
    
    #GTMasks_2 = np.reshape(GTMasks_2,(num,487*487))
    #GTMasks_2 = sparse.csr_matrix(GTMasks_2)
   # print(filename_GT3)
...
def optimize_combine_3(totalmasks, neuronstate, COMs, areas, probmapID, dims, minArea, avgArea, Params_set: dict, filename_GT: str, filename_GT2,  filename_GT3, useMP=True, p=None):
            ...
            for (j1,thresh_COM) in enumerate(list_thresh_COM):
                for (j2,thresh_IOU) in enumerate(list_thresh_IOU):
                    (Recal_k, Precision_k, F1_k) = optimize_combine_1(uniques, times_uniques, dims, 
                        {'avgArea': avgArea, 'thresh_mask': thresh_mask, 'thresh_COM': thresh_COM,
                            'thresh_IOU': thresh_IOU, 'list_cons':list_cons}, filename_GT, filename_GT2, filename_GT3)                                 
...
def parameter_optimization(pmaps: np.ndarray, Params_set: dict, \
        filename_GT: str, filename_GT2, filename_GT3,  useMP=True, useWT=False, p=None): 
                ...
                if useMP:
                    try: 
                        list_result = p.starmap(optimize_combine_3, [(totalmasks, neuronstate, COMs, areas, \
                            probmapID, dims, minArea, avgArea, Params_set, filename_GT, filename_GT2, filename_GT3, False) for minArea in list_minArea])
                    ...
                    for (i1,minArea) in enumerate(list_minArea):
                        ...
                        list_Recall_inter, list_Precision_inter, list_F1_inter = optimize_combine_3(
                            totalmasks, neuronstate, COMs, areas, probmapID, dims, minArea, avgArea, Params_set, filename_GT, filename_GT2, filename_GT3, useMP=False, p=p)
